package tta.astrologerapp.talktoastro.util.logger
...
class Logger : Printer {
    ...
    private val listener = DialogInterface.OnClickListener { dialogInterface, which ->
        ...
        when (which) {
            DialogInterface.BUTTON_NEGATIVE -> {
                ...
                Toast.makeText(context, tta.astrologerapp.talktoastro.util.LogUtils.COPY_SUCCESS, Toast.LENGTH_LONG).show()
            }
    ...
    private fun logObject(type: LogType, element: StackTraceElement, `object`: Any?) {
        if (!tta.astrologerapp.talktoastro.util.LogUtils.ENABLE_LOG) {
            return
        }
        if (`object` != null) {
            ...
            if (`object` is Throwable) {
                ...
                val dim = tta.astrologerapp.talktoastro.util.logger.ArrayUtil.getArrayDimension(`object`)
                when (dim) {
                    1 -> {
                        val pair = tta.astrologerapp.talktoastro.util.logger.ArrayUtil.arrayToString(`object`)
                        msg = simpleName.replace("[]", "[" + pair.first + "] {\n")
                        msg += pair.second.toString() + "\n"
                    }
                    2 -> {
                        val pair1 = tta.astrologerapp.talktoastro.util.logger.ArrayUtil.arrayToObject(`object`)
                ...
                if (!collection.isEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        msg += String.format(
                            itemString, flag, tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString<Any>(item),
                ...
                for (key in keys) {
                    ...
                    val value = map[key]
                    msg += String.format(
                        itemString, tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString(key),
                        tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString<Any>(value)
                    )
                }
                logString(type, element, "$msg}")
            } else {
                logString(type, element, tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString(`object`))
            }
        } else {
            logString(type, element, tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString<Any>(`object`))
        }
    }
    ...
    private fun generateTag(caller: StackTraceElement): String {
        ...
        val Prefix =
            if (TextUtils.isEmpty(tta.astrologerapp.talktoastro.util.LogUtils.configTagPrefix)) "" else tta.astrologerapp.talktoastro.util.LogUtils.configTagPrefix
    ...
    fun alert(context: Context, `object`: Any) {
        ...
        if (null == dialog) {
            dialog = AlertDialog.Builder(context)
                .setNegativeButton(tta.astrologerapp.talktoastro.util.LogUtils.COPY, listener)
                .setPositiveButton(tta.astrologerapp.talktoastro.util.LogUtils.CANCEL, listener)
                .create()

        }
        dialog!!.setMessage(tta.astrologerapp.talktoastro.util.logger.SystemUtil.objectToString(`object`))
        dialog!!.show()
    }
}