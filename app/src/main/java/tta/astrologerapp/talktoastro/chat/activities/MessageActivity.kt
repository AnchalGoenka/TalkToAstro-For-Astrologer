...
import tta.astrologerapp.talktoastro.services.MediaService
...
import tta.astrologerapp.talktoastro.chat.views.MemberViewHolder
import tta.astrologerapp.talktoastro.chat.views.MessageViewHolder
...
class MessageActivity : AppCompatActivity(), ChannelListener, AnkoLogger {
    ...
    val mContext by lazy { this@MessageActivity }

    val mViewModel by lazy { ViewModelProviders.of(mContext).get(ComounViewModel::class.java) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        astroId = SharedPreferenceUtils.readString(
            ApplicationConstant.USERID, "", SharedPreferenceUtils.getSharedPref(
                ApplicationUtil.getContext()))

        createUI()
        chatStatusUpdate("Busy")
    }

    private fun chatStatusUpdate(status: String) {

        val req= ChangStatus(
        ""+status,
        ""+astroId)
        mViewModel.changeChatStatus(req)
            .observe(this, object : Observer<CallHistoryModel> {
                override fun onChanged(resp: CallHistoryModel?) {
        ...
        try {
            if (intent != null) {
                ...
                channelsObject.getChannel(channelSid, ChatCallbackListener<Channel>() {
                    ...
                    val astroName = SharedPreferenceUtils.readString(
                        ApplicationConstant.NAME, "",
                        SharedPreferenceUtils.getSharedPref(
                            ApplicationUtil.getContext())
                    )
                    for (i in members.indices) {
                        if (astroName != members[i].identity) {
                            toolbarMessage.title = members[i].identity
                        }
                    }
                    ...
                    setupListView(channel!!)
//                channel!!.members.inviteByIdentity("User 2", ToastStatusListener(
...
//                    })
            ...
            ApplicationUtil.showDialog(this, "Some error occurred")
        }
    }
        ...
        selector("Select an option", EDIT_OPTIONS) { _, which ->
            when (which) {
                ...
                LIST_MEMBERS -> {
                    ...
                    BaseApplication.instance.showToast("Subscribed users: ${name2.toString()}", Toast.LENGTH_LONG)
                    ...
                    users.getUserDescriptor(channel!!.members.membersList[0].identity, ChatCallbackListener<UserDescriptor>() {
                        BaseApplication.instance.showToast("Random user descriptor: ${it.friendlyName}/${it.identity}", Toast.LENGTH_SHORT)
                    })
                    ...
                    val members2 = BaseApplication.instance.basicClient.chatClient!!.channels.getMembersByIdentity(channel!!.members.membersList[0].identity)
